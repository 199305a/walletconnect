...
import { colors, transitions } from 'src/styles';
...
align-items: center;
`
interface IClouseButtonStyleProps {
    size: number;
    color: string;
    onClick?: any;
}

const SCloseButton = styled.div<IClouseButtonStyleProps>`
...
&:after {
  ...
  height: ${({ size }) => `${size}px`};
  width: 2px;
  background: ${({ color }) => `rgb(${colors[color as keyof typeof colors]})`};
}
&:before {
...
class Modal extends React.Component<IModalProps, IModalState> {
    ...
    public componentDidUpdate() {
        if (this.lightbox) {
            ...
            if (offset !== INITAL_STATE.offset && offset !== this.state.offset) {
    ...
    public toggleModal = async () => {
        ...
        const body = d ? d.body || d.getElementsByTagName("baby")[0] : "";
        if (body) {
            if (this.props.show) {
                body.style.position = "";
            } else {
            ...
            <SLightbox show={show} offset={offset} opacity={opacity} ref={c => (this.lightbox = c)}>
                <SModalContainer>
                    <SHitbox onClick={this.toggleModal}>
                        <SCard>
                            <SCloseButton onClick={this.toggleModal} size={25} color={'dark'}>

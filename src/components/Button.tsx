...
background-color: ${({ outline, color }) => (outline ? "transparent" : `rgb(${colors[color as keyof typeof colors]})`)};
border: ${({ outline, color }) => (outline ? `1px solid rgb(${colors[color as keyof typeof colors]})` : "none")};
color: ${({ outline, color }) => (outline ? `rgb(${colors[color as keyof typeof colors]})` : `rgb(${colors.white})`)};
...
&:disabled {
  ...
  &:active {
    ...
    box-shadow: ${({ outline }) => (outline ? "none" : `${shadows.soft}`)};
    color: ${({ outline, color }) =>
      outline ? `rgb(${colors[color as keyof typeof colors]})` : `rgba(${colors.white}, 0.24)`};
  ...
  & ${SIcon} {
    ...
    background-color: ${({ outline, color }) =>
      outline ? `rgb(${colors[color as keyof typeof colors]})` : `rgb(${colors.white})`};
